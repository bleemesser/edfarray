/// Tolerance, in samples, for treating a time as exactly on the sample grid. It removes
/// floating-point error such as `0.1 + 0.2`, but it does not move a time that is really
/// between samples.
pub const GRID_EPS: f64 = 1e-6;

/// Index of the first sample at or after a time. `samples` is that time, already multiplied
/// by the sample rate.
///
/// Every time-to-sample conversion in the crate uses this one rule. Thus a half-open time
/// range `[start, end)` selects the same samples in every API that resolves it.
